...
- Set `channels.discord.dm.enabled=false` in `openclaw.json` so the bridge becomes the single Discord private-chat ingress path and avoids duplicate sessions
